...
def save_new_nirs(nirs_file, nirs_path, root_export_dir, id):
    ...
    copytree(nirs_path, f"{root_export_dir}/{id}")
    savemat(f"{root_export_dir}/{id}/{id}.nirs", nirs_file)


def main():

    participant = "EML1_055"

    _this_fpath = os.path.dirname(os.path.realpath(__file__))
    ROOT_DIR = Path(f"{_this_fpath}/../data/toolbox_staging/{participant}")
    EXPORT_DIR = Path(f"{_this_fpath}/../data/test_long_stim/")

    nirs = find_files_of_type(ROOT_DIR, ".nirs")
    trigger = find_files_of_type(ROOT_DIR, ".tri")
    nirs_f = loadmat(nirs[participant][0])

    stim_ch_len = nirs_f["s"].shape[0]

    new_stim_data = build_new_stim_array(trigger[participant][0], stim_ch_len)
    nirs_f["s"] = new_stim_data
    save_new_nirs(nirs_f, ROOT_DIR, EXPORT_DIR, participant)
